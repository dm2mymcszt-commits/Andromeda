...
- Current checkpoint: Phase 2. Owner `0cd7066` full CI 34815322711 SUCCESS. F2 `c5cbc37` full CI 34816390584 SUCCESS. F1 `b34cb4f` build 10 compile/profile/live tests passed; UI checks running. Follow-up `e3cc19e` build 11 pushed (CI 34847670535): resolve unknown held destination, cancel unused mode prefetch. Build 12 broadens the held-point fix to retain the pending profile for unknown AND provisional terrain; owner-level tests cover later jumps. Next: push/validate build 12 and all UI checks, then Phase 2 acceptance before Phase 3.
...
| 2 F1 altitude | In progress | Root cause: 10 s lookup + 45 m cache. `b34cb4f` build 10 pushed: batched profile/interpolation, last-known hold, custom preserved, weighted persistent API budget. CI 34847031351 compile/model/live passed; previews pending. `e3cc19e` build 11 follow-up pushed, CI 34847670535 running |
